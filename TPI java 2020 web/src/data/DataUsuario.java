...
public class DataUsuario {
	...
	public String bajaUsuario(Usuario nuevo, Usuario old) {
		...
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement(
					"update usuario set fecha_baja=? where id_usuario=?", PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public ArrayList<Usuario> getAll() {
		...
		try {
			...
			rs = stmt.executeQuery(
					"select id_usuario,usuario,nombre,apellido,tipo_doc,nro_doc,email,is_adoptante,is_donante,localidad,tipo,domicilio,fecha_baja from usuario");
			if (rs != null) {
				while (rs.next()) {
