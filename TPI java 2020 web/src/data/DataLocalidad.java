...
public class DataLocalidad {
	...
	public void setLocalidad(Usuario us) {
		...
		ResultSet rs = null;
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement("select localidad.* " + "from localidad "
					+ "inner join usuario " + "on localidad.id_localidad=usuario.localidad " + "where id_usuario=?");
	...
	public String bajaLocalidad(Localidad l) throws Exception {
		...
		ResultSet keyResultSet = null;
		try {
			stmt = DbConnector.getInstancia().getConn().prepareStatement("delete from localidad where id_localidad=?",
					PreparedStatement.RETURN_GENERATED_KEYS);
			...
			e.printStackTrace();
			throw new Exception(
					"Ha ocurrido un error al dar de baja la localidad, no pueden eliminar localidades activas");
		} finally {
			try {
				if (keyResultSet != null)
					keyResultSet.close();
